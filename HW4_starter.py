# SI 206 HW4
# Your name: Allison Martin
# Your student id: 6434 7054
# Your email: amartinn@umich.edu
...
class Customer:
    ...
    def place_item_order(self, store, order):
        ...
        if total > self.account:
            ...
            self.account = self.account - total
            store.accept_payment(total)
            return True
...
class Store:
    ...
    def calculate_item_cost(self, item, quantity, express_order, customer):
        ...
        if express_order:
            total = total * 1.20
        if customer.employeer_id == self.store_id:
            total = total * .60
        return total
        '''
    ...
    def stock_up(self, item, quantity):
        ...
        if item in self.inventory:
            ...
            self.inventory.__setitem__(item, quantity)

    def process_order(self, order):
        for i in order:
            if i in self.inventory:
                if i.quantity > self.inventory[i].quantity:
                    return False
                else:
                    self.inventory[i].quantity = self.inventory[i].quantity - i.quantity
            else:
                return False
            return True
        '''
...
class TestAllMethods(unittest.TestCase):
    ...
    def test_customer_place_item_order_2(self):
        ...
        self.assertAlmostEqual(ali.account, 3.6, 2) # 15 - (3 + (1.75*4*1.2)) = 15- 11.4 = 3.6
        self.assertAlmostEqual(dunder_mifflin.income, 11.4, 2)
